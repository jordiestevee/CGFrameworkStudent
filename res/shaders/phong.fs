...
void main()
{
    ...
    vec4 specularTexture = texture2D(u_texture, v_uv);

    vec3 flag = vec3(flag.x, flag.y, flag.z);
    if (flag.x == 1.0){
        kd_f = colorTexture.rgb;
        ka_f *= colorTexture.rgb;
        if (flag.y == 1.0){
            ks_f = vec3(specularTexture.a);
        }
        if (flag.z == 1.0){
            normal = normalize(2.0*normal - vec4(1.0));
            vec3 world_normal = (u_model * vec4(normal.xyz, 0.0)).xyz;
            float mix_factor = 0.5;
            N = mix(v_world_normal, world_normal, mix_factor);
        }   
    }
